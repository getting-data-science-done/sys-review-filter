...
def process_data(data_path, title_col, abstract_col, author_col, question_col):
    ...
    df4 = add_question_match_features(df3, question_col, title_col)
    df5 = add_question_match_features(df4, question_col, abstract_col)
    df6 = add_question_match_features(df5, question_col, "TEXT")
    return df6
...
def add_question_match_features(df, q_col, t_col, sfx=""):
    ...
    df_new[[t_col+'_match_prop', t_col+'_jd_'+sfx, t_col+'_ld_'+sfx, t_col+'_ji_'+sfx, t_col+'_sd_'+sfx]] = df_new.apply(q_feats, axis=1, result_type="expand")
    return df_new
...
def add_author_features(df, author_col, sepa=";"):
    ...
    def author_features(x, col):
        try:
           authors = str(x[col])
           auth_array = authors.lower().split(sepa)
